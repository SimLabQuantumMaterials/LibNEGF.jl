    ...
    @testset "Backend Device Transfers" begin
        ...
        for k in [kpoints[1]]
            for precx in [ComplexF64]
                ...
                Bz = be_zero_array(size(Abm.M[1, 1]))
                ...
                Bz = be_zero_array(size(Abm.M[1, 1]))
                Bx = be_copy_to_hw(zeros(size(Abm.M[1, 1])))
                check_if_equal(Bz, Bx, roundoffs[precx], 1.0E0)
                Bz = 0
                Bx = 0
                GC.gc()

                # Bzcpu = zeros(size(Abm.M[1, 1]))
                # Bzdev = be_zero_array(size(Abm.M[1, 1]))
                ...
                # Bzlu = be_zero_lu(size(Abm.M[1, 1])[1])
