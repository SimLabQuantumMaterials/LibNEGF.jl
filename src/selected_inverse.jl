...
"""
function prod_VecOfBlock!(res::Array, A::Array, B::Array, precx, td::TimingData, cd::CountingData)::Nothing
	for i in 1:size(A,1)
...
"""
function blockMatrix_factorization!(A::Matrix, td::TimingData, cd::CountingData)
...
function blockMatrix_factorization_noT!(A::Matrix)
	precx = typeof(A[1, 1].Full[1,1])
	npl = size(A, 1)
	be_zero = convert(precx, 0.0)
	be_one = convert(precx, 1.0)
...
function blockMatrix_factorization_noT(A::Matrix)::Matrix
	...
	return blockMatrix_factorization_noT!(B)
end
...
"""
function blockMatrix_inverse!(A::Matrix, td::TimingData, cd::CountingData)
	precx = typeof(A[1, 1].Full[1,1])
	npl = size(A,1)
	be_zero = convert(precx, 0.0)
	be_one = convert(precx, 1.0)
...
- `NoFillin::Bool` : Flag to know if we consider filled block during the computation.
"""
function blockMatrix_inverse(A::Matrix, td::TimingData, cd::CountingData)::Matrix
	B = bm_copy(A)

	return blockMatrix_inverse!(B, td, cd)
end