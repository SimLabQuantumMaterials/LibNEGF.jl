...
if exists_in_list "$HWs" " " $1; then
    ...
    # already happening. On local systems, e.g., laptops, using this is encouraged
    #JULIA_EXCLUSIVE=1 
    julia --threads=$JULIA_NUM_THREADS runbenchmrks.jl $1 $2 $JULIA_NUM_THREADS $OPENBLAS_NUM_THREADS
else
